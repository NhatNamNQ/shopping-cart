...
export default function Cards() {
    ...
    const greyedStyles = loading && "opacity-30";
                ...
                <div className="flex gap-4">
                            ...
                            {series &&
                                Object.entries(series).map(([seriesName]) => (
                                    <li key={seriesName} className="text-lg mb-1">
                                        <Link to={`/cards/series/${seriesName}`} onClick={handleSeriesChange}>
                                            {seriesName}
                                        </Link>
                                    </li>
                                ))
                    ...
                    <div className="w-full h-full">
                        <ul className="grid grid-cols-1 lg:grid-cols-5 md:grid-cols-3 gap-8">
                            {cards.length ? (
                                cards.map(card => (
                                    <li key={card.id} className={`bg-white rounded-xl shadow-lg hover:shadow-xl ${greyedStyles}`}>
